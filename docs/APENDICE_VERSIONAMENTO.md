...
**Versão corrente documentada:** `0.3.4`.
...
- **Teste de alternativa em branch:** recebe uma letra após a versão de referência, por exemplo `0.3.4a`, `0.3.4b` e `0.3.4c`. A letra identifica uma alternativa experimental e não substitui uma versão integrada à `main`.
...
| `0.3.4` | aperfeiçoamento da ingestão | API autenticada para receber ZIPs, inspecionar itens, gerar manifesto provisório, corrigir metadados e confirmar registros do corpus com controles de segurança. | API de ingestão em lote |
...
```text
0.3.4a  alternativa A para classificação de documentos
0.3.4b  alternativa B para o mesmo ensaio
0.3.4c  terceira alternativa comparável
```

Enquanto o processo de ingestão e recepção documental não estiver superado, aperfeiçoamentos permanecem na série `0.3.x`. Assim, o próximo ajuste integrado será `0.3.5`. A passagem para `0.4.0` fica reservada a uma etapa posterior do roadmap, já além da consolidação da ingestão.
